...
public class EditReplyController implements Controller {

	CommentService commentService;
	
	public EditReplyController() {
	...
	public String handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			String updatedContent = parseJsonValue(body, "content");
			...
			reply.setComment_lay1_no(comment_lay1_no);
			reply.setWritten(updatedContent);
			
			int result = commentService.changeReply(reply);
			if (result > 0) {
				out.print("{\"success\":true, \"message\":\"대댓글 수정 성공\"}");
			} else {
				out.print("{\"success\":false, \"message\":\"대댓글 수정 실패\"}");
			}
